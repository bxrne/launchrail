...
// Motor represents the motor entity of a rocket
type Motor struct {
	ID          int
...
func NewMotor(id int, md *thrustcurves.MotorData) *Motor {
	m := &Motor{
		ID:          id,
...
// String returns a string representation of the Motor
func (m *Motor) String() string {
	return fmt.Sprintf("Motor{ID: %d, Position: %v, Mass: %.3f, Thrust: %.3f, State: %s}", m.ID, m.Position, m.Mass, m.thrust, m.fsm.GetState())
}
