...
func TestCompareFloat(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := bench.CompareFloat(tt.metricName, tt.description, tt.expected, tt.actual, tt.tolerance)
...
func TestFindGroundTruthApogee(t *testing.T) {
	tests := []struct {
		name         string
		data         []bench.FlightInfo
		wantAltitude float64
		wantTime     float64
	}{
		{"Empty Data", []bench.FlightInfo{}, 0, 0},
		{"Single Point", []bench.FlightInfo{{Timestamp: 1.0, Height: 100.0}}, 100.0, 1.0},
		{"Multiple Points Ascending", []bench.FlightInfo{{Timestamp: 1.0, Height: 100.0}, {Timestamp: 2.0, Height: 200.0}}, 200.0, 2.0},
		{"Multiple Points Descending", []bench.FlightInfo{{Timestamp: 1.0, Height: 200.0}, {Timestamp: 2.0, Height: 100.0}}, 200.0, 1.0},
		{"Peak in Middle", []bench.FlightInfo{{Timestamp: 1.0, Height: 100.0}, {Timestamp: 2.0, Height: 300.0}, {Timestamp: 3.0, Height: 200.0}}, 300.0, 2.0},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			alt, time := bench.FindGroundTruthApogee(tt.data)
...
func TestFindGroundTruthMaxVelocity(t *testing.T) {
	tests := []struct {
		...
		{"Empty Data", []bench.FlightInfo{}, 0, 0},
		{"Single Point", []bench.FlightInfo{{Timestamp: 1.0, Velocity: 50.0}}, 50.0, 1.0},
		{"Multiple Points Increasing", []bench.FlightInfo{{Timestamp: 1.0, Velocity: 50.0}, {Timestamp: 2.0, Velocity: 100.0}}, 100.0, 2.0},
		{"Multiple Points Decreasing", []bench.FlightInfo{{Timestamp: 1.0, Velocity: 100.0}, {Timestamp: 2.0, Velocity: 50.0}}, 100.0, 1.0},
		{"Peak in Middle", []bench.FlightInfo{{Timestamp: 1.0, Velocity: 50.0}, {Timestamp: 2.0, Velocity: 150.0}, {Timestamp: 3.0, Velocity: 100.0}}, 150.0, 2.0},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			vel, time := bench.FindGroundTruthMaxVelocity(tt.data)
...
func TestFindGroundTruthEventTime(t *testing.T) {
	tests := []struct {
		name        string
		events      []bench.EventInfo
		targetEvent string
		wantTime    float64
	}{
		{"Empty Events", []bench.EventInfo{}, "APOGEE", -1},
		{"Event Found", []bench.EventInfo{{Timestamp: 5.0, Event: "LAUNCH"}, {Timestamp: 15.2, Event: "APOGEE"}}, "APOGEE", 15.2},
		{"Event Found Case Insensitive", []bench.EventInfo{{Timestamp: 5.0, Event: "LAUNCH"}, {Timestamp: 15.2, Event: "Apogee"}}, "apogee", 15.2},
		{"Event Not Found", []bench.EventInfo{{Timestamp: 5.0, Event: "LAUNCH"}, {Timestamp: 30.0, Event: "LANDED"}}, "APOGEE", -1},
		{"First Event Match", []bench.EventInfo{{Timestamp: 15.2, Event: "APOGEE"}, {Timestamp: 16.0, Event: "APOGEE_DUPE"}}, "APOGEE", 15.2},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			lg := logger.GetLogger("hipr_euroc24_test")
			time := bench.FindGroundTruthEventTime(tt.events, tt.targetEvent, lg)
			assert.Equal(t, tt.wantTime, time)
		})
	}
}
